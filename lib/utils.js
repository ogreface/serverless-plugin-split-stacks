...
module.exports = {
  ...
  getStackName(destination, allowSuffix) {
    ...
    while (stackName === undefined) {
      ...
      this.serverless.cli.log(`[serverless-plugin-split-stacks]: potential stack name: ${potentialStackName}`);
  ...
  stackHasRoom(stack) {
    ...
    this.serverless.cli.log(`[serverless-plugin-split-stacks]: stack resource length: ${Object.keys(resources).length}`);
    this.serverless.cli.log(`[serverless-plugin-split-stacks]: stack output length: ${Object.keys(outputs).length}`);
    this.serverless.cli.log(`[serverless-plugin-split-stacks]: stack paramerers length: ${Object.keys(parameters).length}`);
    
    return Object.keys(resources).length < 58
      && Object.keys(outputs).length < 58
      && Object.keys(parameters).length < 58;
  },
