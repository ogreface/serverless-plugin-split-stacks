...
module.exports = {
  ...
  getStackName(destination, allowSuffix) {
    ...
    while (stackName === undefined) {
      ...
      log("potential stack name: " + potentialStackName);
  ...
  stackHasRoom(stack) {
    ...
    log("stack resource lengths: " + Object.keys(resources).length);
    log("stack outputs lengths: " + Object.keys(outputs).length);
    log("stack parameters lengths: " + Object.keys(parameters).length);
